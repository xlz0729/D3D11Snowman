...
void GeometryGenerator::CreateGeosphere(float radius, UINT numSubdivisions, MeshData& meshData)
{
	...
	numSubdivisions = mMin(numSubdivisions, 5u);
...
void GeometryGenerator::CreateTorus(float diameter, float thickness, UINT tessellation, MeshData & meshData)
{
	meshData.Vertices.clear();
	meshData.Indices.clear();
	...
	{
		float u = 1 - (float)i / tessellation;
		...
		for (size_t j = 0; j <= tessellation; j++)
		{
			...
			vertex.TangentU = XMFLOAT3(0.0f, 0.0f, 0.0f);
			...
			meshData.Indices.push_back(nextI * stride + j);
			meshData.Indices.push_back(i * stride + nextJ);
			meshData.Indices.push_back(i * stride + j);
			
			meshData.Indices.push_back(nextI * stride + j);
			meshData.Indices.push_back(nextI * stride + nextJ);
			meshData.Indices.push_back(i * stride + nextJ);
		}
	}
}